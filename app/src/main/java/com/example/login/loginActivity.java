...
public class loginActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        forget=findViewById(R.id.tv_loginactivity_forget);
        yanzhengma=findViewById(R.id.tv_loginactivity_check);
        loginqq=findViewById(R.id.loginqq);
        loginweixin=findViewById(R.id.loginweixin);
        loginweibo=findViewById(R.id.loginweibo);
        ...
        switch (view.getId()) {
            ...
            case R.id.showPwd:
                if(showPwd.isSelected()){
                    ...
                    mEtLoginactivityPassword.setInputType(InputType.TYPE_TEXT_VARIATION_PASSWORD | InputType.TYPE_CLASS_TEXT );//设置密码不可见
                    Toast.makeText(this,"隐藏密码",Toast.LENGTH_SHORT).show();
                }else{
                    showPwd.setSelected(true);
                    mEtLoginactivityPassword.setInputType(InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);//设置密码可见
                    Toast.makeText(this,"显示密码",Toast.LENGTH_SHORT).show();
                }
                break;
            case R.id.tv_loginactivity_forget:
                Toast.makeText(this,"请联系管理员QQ：2368497009 进行修改",Toast.LENGTH_SHORT).show();
                break;
            case R.id.tv_loginactivity_check:
                Toast.makeText(this,"（◐ˍ◑）点也没用，只是让你看看(✖人✖)",Toast.LENGTH_SHORT).show();
                break;
            case R.id.loginqq:
                Intent intent5 = new Intent();
                intent5.setAction("android.intent.action.VIEW");
                Uri content_url = Uri.parse("https://graph.qq.com/oauth2.0/show?which=Login&display=pc&response_type=code&redirect_uri=http%3A%2F%2Fspcdp.cdposs.qq.com%2Fauth%2Fcallback&client_id=101477813&state=https%3A%2F%2Fspcdp.cdposs.qq.com%2F");//此处填链接
            ...
            case R.id.loginweixin:
                ...
                Uri content_url2 = Uri.parse("http://beian.vhostgo.com/reg/wxlogin.asp");//此处填链接
                intent6.setData(content_url2);
                startActivity(intent6);
                break;
            case R.id.loginweibo:
                Intent intent7 = new Intent();
                intent7.setAction("android.intent.action.VIEW");
                Uri content_url3 = Uri.parse("https://passport.weibo.cn/signin/login?r=http%3A%2F%2Fnew.vip.weibo.cn%2F");//此处填链接
                intent7.setData(content_url3);
